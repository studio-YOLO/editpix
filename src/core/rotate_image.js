 ...
 * @returns {number[]} rotatedPixelArray: Rotated image pixel array in the format [R, G, B, alpha,..., R, G, B, alpha]
 */
function rotateImage(pixelArray, angle) {
    const radians = angle * Math.PI / 180;
    const cosTheta = Math.cos(radians);
    const sinTheta = Math.sin(radians);

    const width = Math.sqrt(pixelArray.length / 4);
    const height = width;
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (source_x >= 0 && source_x < width && source_y >= 0 && source_y < height) {
                ...
                const R1 = pixelArray[index1] * (1 - tX) + pixelArray[index2] * tX;
                const R2 = pixelArray[index3] * (1 - tX) + pixelArray[index4] * tX;
                const R = R1 * (1 - tY) + R2 * tY;

                const G1 = pixelArray[index1 + 1] * (1 - tX) + pixelArray[index2 + 1] * tX;
                const G2 = pixelArray[index3 + 1] * (1 - tX) + pixelArray[index4 + 1] * tX;
                const G = G1 * (1 - tY) + G2 * tY;

                const B1 = pixelArray[index1 + 2] * (1 - tX) + pixelArray[index2 + 2] * tX;
                const B2 = pixelArray[index3 + 2] * (1 - tX) + pixelArray[index4 + 2] * tX;
                const B = B1 * (1 - tY) + B2 * tY;

                rotatedPixelArray.push(R);
                rotatedPixelArray.push(G);
                rotatedPixelArray.push(B);
                rotatedPixelArray.push(pixelArray[index1 + 3]);
            } else {
                rotatedPixelArray.push(0); // Red
                rotatedPixelArray.push(0); // Green
                rotatedPixelArray.push(0); // Blue
                rotatedPixelArray.push(0); // Alpha
            }
        }
    }
